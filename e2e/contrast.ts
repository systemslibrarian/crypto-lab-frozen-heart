 ...
 *    files the node under `incomplete`. Nearly every tinted surface on this page
 *    is a `color-mix(in oklab, ...)` wash over a panel — `.subst`, every
 *    `.guide-banner` tone, the `.rung` severity tints, `.preset.active`'s ring —
 *    so their text composites onto a computed colour rather than onto the flat
 *    value axe requires.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The formula tokens animate
 *    through `opacity: 0` as a field drops out of the hash, and the
 *    `.indicator.flash` verdict pulse runs over live text.
 *
 ...
 *    several `overflow-x: auto` scrollers around unbreakable content — the
 *    `.formula` hash expression, the `.codeblock` audit snippets, the `.eq-hex`
 *    sides of the verification equation — and at 380px a good deal of that is
 *    scrolled out of its client box. Content past the client box is not dimmed
 *    or partly drawn: it is absent from the frame, and asking what colour it
 *    sits on has no answer. Its rect is outside every ancestor's box, so the
 *    walk would find nothing behind it and fall through to white, inventing a
 *    failure. Skip it, and rely on the wider viewport where the very same
 ...
 *    draws no SVG figures — its sequence diagram is HTML boxes — but the rule is
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * and status fills into surfaces with `color-mix(in oklab, ...)` throughout
     * — `.subst`, `.guide-banner`, the `.rung` and `.cmp-col` tints — and the
     * shared header bar uses `color-mix(in srgb, ...)`. Chromium reports those
     ...
     *
     * This page renders five omission-ladder rungs, each with three measurement
     * chips and a full commentary paragraph, plus a hex chip per proof value and
     * a token per hash field — hundreds of text nodes all walking the same
     * handful of `.card` / `.rung` ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures. This page leans on the
     * idiom hard: every truncated hex value on screen is an `aria-hidden` short
     * form beside an `.sr-only` span carrying the FULL value, and `.sr-only`
     * here is the classic `clip: rect(0, 0, 0, 0)` recipe. Without this the walk
     * measures each of those hidden twins. Deliberately narrow — only a ZERO-area clip
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page ships seven closed `<details>` — the notation
      // primer, the Schnorr-signature aside, the custom-transcript toggles, the
      // two ladder demonstrations, the random-oracle note and the references —
      // and the gate opens and scans each one explicitly.
     ...
     * it, this page's decorative glyphs — the ✓/✗/⚠/🛡 `.badge-icon`s, the ▶/◀
     * `.flow-arrow`s, the ①②③ `.algebra-num`s, the `− R` `.cmp-delta` — are each
     * measured as if the glyph took the element's declared `color`. An emoji
